...
class DIC(ExperimentalDesign):
    def __init__(self, data: pd.DataFrame, response: str, treatment: str):
        """
        Inicializa um modelo de DIC (Delineamento Inteiramente Casualizado).
    ...
    def anova(self) -> pd.DataFrame:
        """
        Executa a análise de variância (ANOVA) para o delineamento DIC.

        Returns:
            DataFrame com a tabela ANOVA (Tipo II).
        """
        formula = f"{self.response} ~ C({self.treatment})"
    ...
    def posthoc(self, method: str = "tukey") -> pd.DataFrame:
        ...
        Parameters:
            method: Método de comparação múltipla (atualmente apenas "tukey" suportado)
